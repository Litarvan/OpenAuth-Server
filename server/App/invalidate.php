...
if($request['method'] == "POST"){
	if($request['content-type'] == "application/json"){
		...
		if(!is_null($accessToken) && !is_null($clientToken)){
			$req = Core\Queries::execute('SELECT * FROM users WHERE clientToken=:clientToken', ['clientToken' => $clientToken]);
			if(!empty($req)){
				if($accessToken == $req->accessToken){
					Core\Queries::execute("UPDATE users SET accessToken=:accessToken WHERE clientToken=:clientToken", ['clientToken' => $clientToken, 'accessToken' => '']);
				}else{
					echo error(4);
				}
			}else{
